...
class AustraliaPostTest < Test::Unit::TestCase
  ...
  def test_domestic_response
    response = @carrier.find_rates(@melbourne, @sydney, @packages[:case_of_wine])
  ...
  def test_domestic_combined_response_prices
    ...
    response_small_half_pound = @carrier.find_rates(@melbourne, @sydney, @packages[:case_of_wine])
  ...
  def test_international_failed_message
    ...
    assert_match /The maximum weight of a parcel is 22 kg/, error.message
  end
